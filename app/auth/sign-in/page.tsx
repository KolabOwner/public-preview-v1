    ...
    try {
      ...
      const unsubscribe = onAuthStateChange(async (currentUser) => {
        if (currentUser) {
          ...
          // Create user document if it doesn't exist (for first-time social login)
          // or update last login time
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="text-right">
              <Link href="/auth/forgot-password" className="text-[#48c9b0] hover:text-[#16a085] text-sm transition-colors">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="relative flex items-center justify-center font-bold uppercase focus:ring-0 focus:outline-none transition transition-200 whitespace-nowrap w-full border-0 bg-[#48c9b0] active:bg-[#16a085] focus:bg-[#48c9b0] text-white hover:bg-[#16a085] px-3 py-2.5 h-10 leading-5 rounded-md text-[14px] tracking-[0.14px] disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
          ...
          <div className="mt-6 text-center">
            ...
            <Link href="/auth/sign-up" className="text-[#48c9b0] hover:text-[#16a085] transition-colors">
              Sign Up
            </Link>
          </div>
        </motion.div>
